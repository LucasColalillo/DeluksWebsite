...
import { IUiEducative } from "@/utils/graphqlInterface";
import Link from "next/link";
import { useEffect, useRef, useState } from "react";
import { PlayCircleIcon } from '@heroicons/react/24/outline'

export function UiEducative(props: {
    data: IUiEducative
}) {
    const [activeTab, setActiveTab] = useState(props.data.educativesCollection.items[0].title);
  ...
  return (
    <div id='services' className="bg-[#240131] relative">
      ...
      {props.data.educativesCollection.items.map(tab => (
        <div 
            key={tab.title}
        ...
        {props.data.educativesCollection.items.map(tab => (
          ...
          className='tab-content pb-8 min-h-[40vh] md:min-h-[60vh] lg:min-h-[80vh] flex items-center text-center flex-col md:text-start md:flex-col md:items-baseline' data-tab-name={tab.title} /* style={{minHeight: '80vh'}} */>
            ...
            <div className="video-container">
                                <iframe src={tab.video} allow="autoplay" allowFullScreen></iframe>
                            </div>
            </div>
          </div>
        ))}
    ...
    <div className="pb-12 flex justify-center rounded-xl shadow">
                ...
                "fontSize": "30px",
                "height": "99px",
                }} className="py-3 px-4  bg-[#8C52FF] hover:bg-indigo-700 focus:ring-indigo-500 focus:ring-offset-indigo-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-xl">
